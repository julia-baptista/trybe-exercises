...
const toCelsious = (degreeCelsius) => degreeCelsius;
...
const sendMarsTemperature = (onSucsses, onError, type) => {
  const originalTemperature = getMarsTemperature();
  const currentTemperature = type(originalTemperature);
  const success = Math.random() < 0.6;
  setTimeout(() => {
    if (success) {
      onSucsses(currentTemperature);
    } else {
...
sendMarsTemperature(temperatureInFahrenheit, handleError, toFahrenheit);

// imprime "Hi there! Curiosity here. Right now is 53ºC at Mars", por exemplo, ou "Error getting temperature: Robot is busy"
sendMarsTemperature(greet, handleError, toCelsious);
